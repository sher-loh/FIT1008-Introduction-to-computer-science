...
def check_input(charm: int, bulb: int, squir: int, missingno: int) -> bool:
    """
    ...
    Complexity: O(1)
    """
    # The total numbers of normal Pokemon cannot be greater than 6
    if charm + bulb + squir + missingno > 6:
        raise ValueError("The team cannot have more than 6 Pokemon")
    # MissingNo only can have 0 or 1
    elif missingno > 1:
        raise ValueError("MissingNo cannot be more than 1")
    # Each number of Pokemon type cannot be negative
    elif charm < 0 or bulb < 0 or squir < 0 or missingno < 0:
        raise ValueError("The number of Pokemon cannot be negative")
    # Total number of pokemon in team cannot be 0
    elif charm + bulb + squir + missingno == 0:
    ...
    def choose_team(self, battle_mode: int, criterion: str = None) -> None:
        ...
        while not valid_criterion:
            try:
                ...
                print(
                    "Not a valid criterion, criterion can only be Level,"
                    " HP, Attack,Defense or Speed. Please enter input again")
                criterion = str(input())
            else:
        ...
        while not valid_team:
            try:
                ...
                no_of_missingno = lst[3]  # The 4th element in lst is the number of missingno
                check_input(no_of_charm, no_of_bulb, no_of_squir, no_of_missingno)
            except ValueError:
                print(
                    "The team can only have 1 to 6 Pokemon and the number of "
                    "Pokemon can only be positive integer. Please try again.")
            except IndexError:
                print("Invalid input, input must be in the form of C B S M. Please try again.")
            else:
                self.assign_team(no_of_charm, no_of_bulb, no_of_squir, no_of_missingno)
                valid_team = True

    def assign_team(self, charm: int, bulb: int, squir: int, missingno: int) -> None:
        """
        After input is valid, team will store the charm, bulb, squir and missingno pokemons in different ADT depending on battle_mode.
        :param self:
        :param charm:
        :param bulb:
        :param squir:
        ...
        if self.battle_mode == 0:
            self.team = ArrayStack(charm + bulb + squir + missingno)
        ...
        # If battle_mode is 1 -> Rotating Mode battle (CircularQueue ADT)
            ...
            # If battle_mode is 2 -> Optimised Mode Battle (List ADT)
            self.team = ArraySortedList(charm + bulb + squir + missingno)
            ...
            for i in range(missingno):
                # creating a MissingNo object
                m = MissingNo()
                # MissingNo constant = 0 , key of MissingNo is always set to be the last element
                self.team.add(ListItem(m, int(1000 - (m.get_criteria(self.criterion)*0.1 + m.get_constant()))))
