""" task2_tester.py: The tester for task 2."""
...
class TestTask2(TesterBase):
    ...
    def test_limit(self):
        """
        1. Test instantiating an object
        2. input is valid or not, 4 4 1 will fail (error because greater than 6)
        3. Passing argument from one object over another
        if valid will pass to assign_team and assign the team
        4. If assign success
        """
        ...
        try:
            assert str(
                team) == "Charmander's HP = 7 and level = 1, Bulbasaur's HP = 9 and " \
                         "level = 1, Squirtle's HP = 8 and level = 1"
        except AssertionError:
