...
class PokemonBase(ABC):
    ...
    def set_poke_type(self, poke_type: str) -> None:
        ...
        complexity: O(1)
        """
        if poke_type in ["GRASS", "FIRE", "WATER"]:
            self.poke_type = poke_type
        else:
            raise TypeError("PokeType is invalid, PokeType must be either GRASS, FIRE or WATER")
    ...
    def get_poke_name(self) -> str:
        """
        An abstract method to get the name of a pokemon
        :param self:
    ...
    def get_constant(self) -> int:
        """
        An abstract method to get the constant value that is set for each pokemon
        :param self:
